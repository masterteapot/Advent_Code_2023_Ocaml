...
let find_smudge a_str b_str =
    ...
    let rec aux loop_counter mismatch_counter mismatch_index = (
        if loop_counter = ln && mismatch_counter = 1 then mismatch_index 
        else if loop_counter = ln then -1
        else if a_str.[loop_counter] <> b_str.[loop_counter] then aux (loop_counter + 1) (mismatch_counter + 1) loop_counter
        else aux (loop_counter + 1) mismatch_counter mismatch_index)
    in
    let mismatch_score = aux 0 0 0 in
    let result = match mismatch_score with
    | -1 -> false, a_str, b_str
    | n -> true, (switch_char n a_str b_str), b_str in
    result
;;
...
let find_solution_p2 input =
  let rec aux input acc =
    match input with
    | hd :: md :: tl when 
        Tuple3.first (find_smudge hd md) ->
