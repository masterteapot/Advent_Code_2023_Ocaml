...
let parse_inputs max_values g_string =
    let update_game a_game a_pick =
        ...
        let p_group = match pick with
        | num :: color :: [] -> (num |> String.strip |> int_of_string), color
        | _ -> failwith "this doesn't look right" in
        match p_group with
    ...
    let g_index = String.split_on_char ' ' (List.hd g_split) |> List.last |> int_of_string in
    let g_rounds = List.last g_split |> String.split_on_char ';' in
    let g_picks = List.map (fun x -> String.split_on_char ',' x) g_rounds |> List.flatten |> List.map String.strip in
    let g = {index = g_index; max_blue = 0; max_red = 0; max_green = 0; is_possible = true} in
    List.iter (update_game g) g_picks;
    if 
        g.max_blue > max_values.blue
...
let get_powers a_game =
    a_game.max_red * a_game.max_blue * a_game.max_green


let in_1 = read_lines "inputs/2.txt" |> remove_empty_string |> List.map (parse_inputs my_max)
let in_2 = read_lines "inputs/2.txt" |> remove_empty_string |> List.map (parse_inputs my_max)
